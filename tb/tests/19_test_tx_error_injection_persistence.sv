class test_tx_error_injection_persistence extends base_test;
function new( virtual valid_ready_if #(8,8,1)  v_cfg,
 ...
 virtual task configure_dut();
    $display("[%0t] [TEST 19] Configuring DUT.", $time);
  ...
  virtual task main_test();
    $display("[%0t] [TEST 19] Executing test_tx_error_injection_persistence.", $time);
    $display("=================================================");

    $display("[%0t] [TEST 19] Generating TX Transactions without flipping byte parity .", $time);
    
    #320000;
        gen_vr_tx.send_tx_traffic(2, 0, 0, 0);
$display("[%0t] [TEST 19] Generating TX Transactions  flipping byte parity .", $time);
    ...
    $display("[%0t] [TEST 19] Traffic Generated. Waiting for serialization.", $time);
