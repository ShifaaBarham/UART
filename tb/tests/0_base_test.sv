class base_test;
  ...
  virtual UART_if        #(32)      vif_uart_tx;
  virtual UART_if        #(32)      vif_uart_rx;
...
mailbox #(vr_Transaction #(8,8,1))  mbx_gen2drv_config;
  ...
  function new( virtual valid_ready_if #(8,8,1)  v_cfg,
    ...
    cfg_uart_tx = new(); cfg_uart_tx.vif = vif_uart_tx; cfg_uart_tx.is_active = 0; cfg_uart_tx.is_tx = 1; cfg_uart_tx.agent_type = uart_agent_pkg::SLAVE;
    cfg_uart_rx = new(); cfg_uart_rx.vif = vif_uart_rx; cfg_uart_rx.is_active = 1; cfg_uart_rx.is_tx = 0; cfg_uart_rx.agent_type = uart_agent_pkg::MASTER;
  ...
  fork
    ...
    begin 
     ...
     begin
       repeat(watchdog_limit) #1;
                $display("=================================================");

        $display("[%0t] [FATAL] WATCHDOG TIMEOUT! Test hung.", $time);
        $display("=================================================");
        $fatal(1, "Simulation killed by Watchdog!");
      end  
    join_any
    disable fork;
