class test_tx_ppb_parity_sanity extends base_test;
 ...
 virtual task configure_dut();
    $display("[%0t] [TEST 15] Configuring DUT...", $time);
  ...
  virtual task main_test();
    ...
    $display("[%0t] [TEST 15] Generating 5 TX Transactions...", $time);
    gen_vr_tx.send_tx_traffic(5, 0, 0, 0);

    $display("[%0t] [TEST 15] Traffic Generated. Waiting for serialization...", $time);
